...
void loop() {
  ...
  lcd.print("LECTURA VOLTAJE");   // Escreve no LCD  
  ...
  if(Key>='A' && Key<='C'){
    ...
    lcd.clear();
    lcd.home();         // Coloca el cursor en las coordenadas (0,0)   
    lcd.print("Digite Password:"); // Escribe no LCD   
    PW=password();
    //Valida a senha
    if(PW == CurrentPW){
      if(KeyOp=='A'){ //Gravar 4 valores de Voltagem
        lcd.clear();
        lcd.home();         // Coloca el cursor en las coordenadas (0,0)   
        lcd.print("Grabando..."); // Escribe no LCD 
      ...
      if(KeyOp=='B'){ //Trocar a Senha
        lcd.clear();
        lcd.home();         // Coloca el cursor en las coordenadas (0,0)   
        lcd.print("Nuevo Pasword:"); // Escribe no LCD 
        PW=password();
        EEPROM.put( 0, PW );  //Grava na EEPROM
        lcd.clear();
        lcd.home();         // Coloca el cursor en las coordenadas (0,0)   
        lcd.print("Password Grabado!!!"); // Escribe no LCD
        CurrentPW=PW;
        delay(2000);
      }
